...
cust_Id = input()
print("Enter the New sim number")
sim_num = input()
...
try:
    ...
    class SimChange ():
        def change(self):
            ...
            time.sleep(15)
            driver.find_element_by_xpath("(//div[@data-cy='overviewprofile'])[1]").click()
            driver.find_element_by_xpath("(//div[@data-cy='Overviewservice'])[1]").click()      #mtng only
            time.sleep(25)
            ...
            print("Initiated SimChange")
            # more = driver.find_element_by_xpath("(//div[@data-cy='moreActions'])")
            # more.click()
            time.sleep(10)
            action = driver.find_element_by_xpath("(//span[text()='SIM Change'])")
            action.click()
            time.sleep(20)
            ...
            time.sleep(8)
            driver.find_element_by_xpath("//*[input[@value='simLost']]").click()
            time.sleep(2)
            driver.find_element_by_xpath("//button[@data-cy='Next']]").click()
            time.sleep(2)
            driver.find_element_by_xpath("//button[@data-cy='Next']]").click()
            time.sleep(3)
            driver.find_element_by_xpath("//input[@name='newSimNumber]").send_keys(sim_num)
            time.sleep(2)
            ...
            driver.find_element_by_xpath("(//input[@name='reason'])").send_keys("Later")
            time.sleep(8)
