...
search_customer = driver.find_element_by_xpath("//input[@placeholder='Search for Customers']").send_keys("C2518")
time.sleep(10)
...
try:
    ...
    class Termination ():
        def pre(self):
            ...
            time.sleep(15)
            driver.find_element_by_xpath("(//div[@data-cy='Overaccount'])[1]").click()
            time.sleep(25)
            ...
            # more.click()
            time.sleep(10)
            action = driver.find_element_by_xpath("(//span[text()='Termination'])").click()
            time.sleep(20)
            desc = driver.find_element_by_name("description").send_keys("abcdefgh")
            time.sleep(2)
            # driver.find_element_by_xpath("(//button[@type='submit'])[3]").click()
            driver.find_element_by_xpath("(//button[@type='submit'])[3]").click() #mtng only #Localhost
            time.sleep(8)
